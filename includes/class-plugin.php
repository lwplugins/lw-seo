...
final class Plugin {
    ...
    private function output_singular_meta(): void {
        ...
        // Get meta values.
        $title       = Options::get_post_meta( $post->ID, 'title' ) ?: get_the_title( $post );
        $description = $this->get_meta_description( $post );
        $canonical   = Options::get_post_meta( $post->ID, 'canonical' ) ?: get_permalink( $post );

        // Get OG specific values.
        $og_title       = Options::get_post_meta( $post->ID, 'og_title' ) ?: $title;
        $og_description = Options::get_post_meta( $post->ID, 'og_description' ) ?: $description;
    ...
    private function output_home_meta(): void {
        ...
        $description = Options::get( 'desc_home' ) ?: get_bloginfo( 'description' );
        $url         = home_url( '/' );
    ...
    private function output_taxonomy_meta(): void {
        ...
        $title       = single_term_title( '', false ) ?: $term->name;
