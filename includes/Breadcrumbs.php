...
final class Breadcrumbs {
	 ...
	 *
	 * @param array<string, mixed> $args Arguments (unused, kept for consistency).
	 * @return void
	 */
	private function add_date( array $args ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		if ( is_year() ) {
			$this->items[] = [
	...
	private function add_search( array $args ): void {
		if ( $args['show_current'] ) {
			...
			$search_title  = sprintf( __( 'Search: %s', 'lw-seo' ), get_search_query() );
			$this->items[] = [
				'title' => $search_title,
				'url'   => '',
			];
